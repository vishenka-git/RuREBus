...
class Ner(BertForTokenClassification):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, valid_ids=None,
                attention_mask_label=None, device=torch.device('cuda')):
...
class DataProcessor(object):
    ...
    def get_dev_examples(self, data_dir, labels_list):
    ...
    def _read_tsv(cls, input_file):
        ...
        data = [(ex['token'], ex['tag'][:len(ex['token'])], ex['id']) for ex in data]
        return data
...
class NerProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, f"train.json")), "train")

    def get_dev_examples(self, data_dir, labels_list):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, f"dev.json")), "dev")
    ...
    def get_tag_labels(self, data_dir):
        """See base class."""
        dataset = self._read_tsv(os.path.join(data_dir, "train.json"))
        denominator = len([tag for (_, tags, ex_id) in dataset for tag in tags])
        count = Counter()
        labels = []
        for (_, tags, ex_id) in dataset:
            for tag in tags:
                count[tag] += 1
        ...
        for label, count in count.most_common():
            ...
            if label not in labels:
                labels.append(label)
        return labels + ['[CLS]', '[SEP]']

    def _create_examples(self, lines, set_type):
        examples = []
        for i, (sentence, label, ex_id) in enumerate(lines):
...
def main():
    ...
    global EVAL_TAGS
    EVAL_TAGS = [label for label in label_list if label not in ['O', '[CLS]', '[SEP]']]
    ...
    if args.do_validation:
        ...
        all_valid_ids = torch.tensor([f.valid_ids for f in dev_features], dtype=torch.long)
        all_lmask_ids = torch.tensor([f.label_mask for f in dev_features], dtype=torch.long)
        dev_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_valid_ids,
                                 all_lmask_ids)
        dev_sampler = SequentialSampler(dev_data)
        dev_dataloader = DataLoader(dev_data, sampler=dev_sampler, batch_size=args.eval_batch_size)
        validation_steps = int(
            len(train_examples) / args.train_batch_size) // args.validate_per_epoch
    if args.do_train:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                loss = model(input_ids, segment_ids, input_mask, label_ids, valid_ids, l_mask, device=device)
                if n_gpu > 1:
                ...
                if args.do_validation and (step + 1) % validation_steps == 0:
                    ...
                    for batch in tqdm(dev_dataloader, desc='Validation'):
                        ...
                        input_ids, input_mask, segment_ids, label_ids, valid_ids, l_mask = batch
                        with torch.no_grad():
                            logits = model(input_ids, segment_ids, input_mask, None, valid_ids, l_mask, device=device)
                    ...
                    y_true_copy = [[x if x in allowed_tags else 'O' for x in y] for y in y_true]
                    y_pred_copy = [[x if x in allowed_tags else 'O' for x in y] for y in y_pred]
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        args.test_file = os.path.join(args.data_dir, 'test.json') if args.test_file == '' else args.test_file
        ...
        logger.info("\n%s", report)
        output_eval_file = os.path.join(args.output_dir, f"{args.test_file.split('/')[-1]}_eval_results.txt")
        output_preds_file = os.path.join(args.output_dir, f"{args.test_file.split('/')[-1]}_predictions.tsv")
        ...
        prediction_results = {
            ...
            'scores': [' '.join([str(x) for x in score]) for score in tag_scores]
        }
        pd.DataFrame(prediction_results).to_csv(output_preds_file, sep='\t', index=False)
